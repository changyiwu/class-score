...
先切到要抽籤的班級，再點頁首的「**幸運抽籤**」。名單就是目前班級的所有座號（試算表增刪座號後，切換一次班級分頁即同步）。

- **抽籤方式**：幸運大轉盤、滾動拉霸機、神秘翻牌（翻牌模式由老師直接點卡牌，「開始抽籤」只負責洗牌）
- **一次抽 N 位**：N 位都是動畫實際抽出來的，不是抽一位再隨機補
- **不重複抽**：勾選後中籤者會移出這一輪的名單；全部抽完時按「重置名單」再來一輪
- **中籤後加分**：中籤畫面每位都有「加 1 分」，多位時另有「全部加 1 分」。效果與點學生卡片的 `＋` 完全相同（寫入試算表、記進操作紀錄、失敗自動復原），按過的會標示「已加 N」
- 抽籤名單（這一輪抽過誰）、抽籤紀錄與抽籤設定存在**這台電腦的瀏覽器**裡，登出、重新整理、關機後都還在；換電腦或換瀏覽器就各自獨立。要從頭來過就按「重置名單」，紀錄旁的「清空」可清掉紀錄
- 為了保護個資，瀏覽器裡**只存座號與時間**，不存姓名；姓名要登入後才會顯示在紀錄上
