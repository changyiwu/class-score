...
點班級標題右側的「**編輯座號**」。格子中每個座號可點擊切換「有／無」：
...
「最大座號」欄位可擴大或縮小範圍：調大後面會多出可勾選的座號，調小則超出範圍的座號會被標為移除。

對話框下方會即時列出將新增與移除哪些座號，**移除的部分會顯示該生姓名與目前分數**，方便確認是否誤選。按下儲存時若有移除，會再跳出一次確認。

- 留任學生的姓名與分數**完全不受影響**
- **移除座號會一併刪除該生的姓名與分數，無法復原**（但會留在 `_Log` 中可查）
- 班級至少要保留一個座號
...
點「**操作紀錄**」可查看何時加了誰的分。系統會記錄加減分、建立班級、新增座號、移除座號，存放在 `ClassScoreDB` 的 `_Log` 分頁。

- 預設只顯示目前班級，可勾選「顯示所有班級」
- 移除座號的紀錄會保留該生當時的分數，方便誤刪後回查
...
| 學期中有學生轉入／轉出 | 點班級標題右側的「編輯座號」新增或移除座號 |
| 誤刪座號想救回分數 | 點「操作紀錄」，移除紀錄中有該生當時的分數，再用「編輯座號」加回座號並手動補分 |
