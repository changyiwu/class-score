...
到 `ClassScoreDB` 對應的班級分頁直接編輯：

- **新增座號**：在最後補一列，填入 `座號 / 姓名 / 0`
- **移除座號**：刪掉整列
- 座號不必手動排序，大螢幕載入時會依座號由小到大排列
...
點「**操作紀錄**」可查看何時加了誰的分。系統會記錄加減分與建立班級，存放在 `ClassScoreDB` 的 `_Log` 分頁。

- 預設只顯示目前班級，可勾選「顯示所有班級」
- 「裝置」欄是登入 token 的前 8 碼，用來區分不同登入裝置／時段。由於系統只有單一共用密碼，**無法識別是哪一位教師**
- 紀錄超過 5000 筆時會自動從最舊的開始裁切
...
`clasp push` 只是上傳原始碼，**必須再建立新版本才會生效**。本機 clasp 為 3.x，指令是：

```bash
clasp redeploy <既有的部署ID> -d "更新說明"
```

（`redeploy` 是 `update-deployment` 的別名；clasp 2.x 的寫法是 `clasp deploy --deploymentId <id>`。用 `clasp list-deployments` 可查現有部署。）

或在編輯器中「部署 → 管理部署作業 → 編輯（鉛筆圖示）→ 版本選『新版本』→ 部署」。

**請務必沿用同一個部署 ID**，網址才不會改變；若用 `clasp deploy` 建立了全新部署，記得同步更新 `app.js` 的 `GAS_API_URL`。
...
| 學期中有學生轉入／轉出 | 到 `ClassScoreDB` 對應分頁新增或刪除整列 |
| 想重設整學期分數 | 到 `ClassScoreDB` 對應分頁，把 C 欄整欄改回 0 |
| 想刪除班級 | 到 `ClassScoreDB` 刪除該分頁 |
| 分數被誤改，想查何時改的 | 點「操作紀錄」，或直接看 `ClassScoreDB` 的 `_Log` 分頁 |
| `_Log` 分頁太大想清空 | 直接刪除整個 `_Log` 分頁即可，系統會在下次記錄時自動重建 |
