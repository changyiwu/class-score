...
點班級標題右側的「**編輯姓名**」，在對話框中逐一填入姓名後儲存。

- 欄位**留空**即還原為預設名稱，卡片會退回只顯示座號
- 座號與分數不會被更動
- 也可以照舊到 `ClassScoreDB` 直接編輯 B 欄，但請保持 A 欄（座號）與 C 欄（分數）不動

### 重設分數

點「**重設分數**」，確認後將本班所有學生分數歸零。姓名與座號保留。適合每學期重新開始。

**分數無法復原**，執行前請先確認是否需要到 `ClassScoreDB` 備份（複製整個分頁即可）。

### 刪除班級

點「**刪除班級**」。由於此操作不可復原，必須**逐字輸入班級名稱**才能按下確認。刪除後該分頁會從 `ClassScoreDB` 移除。

### 查看操作紀錄

點「**操作紀錄**」可查看誰在何時加了誰的分。系統會記錄加減分、建立班級、重設分數、刪除班級、改姓名等操作，存放在 `ClassScoreDB` 的 `_Log` 分頁。
...
| 想重設整學期分數 | 點班級標題右側的「重設分數」 |
| 想刪除班級 | 點「刪除班級」，需逐字輸入班級名稱確認 |
| 分數被誤改，想查是誰改的 | 點「操作紀錄」，或直接看 `ClassScoreDB` 的 `_Log` 分頁 |
