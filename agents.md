...
2026-09-11 併入原 `exam-review`（考卷檢討）專案：登入後可從頁首「考卷檢討」開啟 Drive 考卷資料夾的檔案清單，兩者共用同一套登入。`exam-review` 預定退場。
...
- [ ] 考卷檢討**真人登入實測**：列清單、展開資料夾、點檔開新分頁（Agent 沒有密碼，只用假後端驗過前端）
- [ ] `exam-review` 退場：舊網址轉址、`teaching-web` 側邊欄連結改指本站、GitHub repo 封存、舊 GAS 部署停用、`考卷檢討-後台` 資料夾與設定試算表處置（每一步都對外，逐項確認再做）
- [ ] 功能面待辦：PWA manifest
...
- **`exam-review` 原本的 `考卷檢討-後台` 資料夾與 `考卷檢討設定` 試算表已不再被本系統讀取**；密碼、登入時長一律以 `ClassScoreDB` 的 `_Settings` 為準
...
- **頁首寬度是有預算的**：`.tabs-container` 的 `max-width` 設為 `40vw`，是為了讓出空間給右側的按鈕、計時器與登出鈕。再往頁首加東西前，先在 1920 與 1366 兩種寬度、8 個班級的情況下量過不會擠掉登出鈕
- **頁首右側目前是「課堂資訊 ▾」選單＋「考卷檢討」＋計時器＋登出**。資訊圖表一律加進 `#infographic-menu` 當選單項目，**不要再把圖表拆回獨立按鈕**。2026-09-11 實測（8 個班級）：1920 下分頁列與右側區塊間距 265px；1366 下分頁列頂到 `40vw` 並內部捲動，與右側間距 27px，登出鈕維持單行（改版前 4 顆按鈕時間距為 0、登出鈕折成兩行）
