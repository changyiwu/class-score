...
- 2026-09-04：新增第四張資訊圖表「課堂表現加減分」，前後端白名單與頁首按鈕一併補上（**目前部署版本 GAS `@10`**）
...
- [ ] 頁首第五顆按鈕**不要再加**：1366 寬、班級數多時分頁列已頂到上限並與按鈕貼齊（見〈UI/UX 設計〉）
- [ ] 功能面待辦：PWA manifest
...
- **頁首寬度是有預算的**：`.tabs-container` 的 `max-width` 設為 `40vw`，是為了讓出空間給右側的資訊圖表按鈕、計時器與登出鈕。再往頁首加東西前，先在 1920 與 1366 兩種寬度量過不會擠掉登出鈕。**2026-09-04 實測（4 顆按鈕）**：1920 下分頁列與右側區塊還有 288px 餘裕；1366 下若有 8 個班級，分頁列已頂到 `40vw` 上限並內部捲動、與第一顆按鈕間距為 0，登出鈕折成兩行但仍完整可見——**這是預算的極限，不要再加第五顆按鈕**（此狀態在只有 3 顆按鈕時就已發生，加第四顆並未讓它變壞）
