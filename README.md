...
6. **操作紀錄**：所有加減分與建立班級寫入 `_Log` 分頁，可在介面查詢何時加了誰的分，出爭議時可追溯。
7. **幸運抽籤**：頁首一鍵開啟，名單直接取目前班級，支援轉盤／拉霸／翻牌、一次抽多位、不重複抽；抽出後可直接替中籤座號加分。

## 目錄結構
- `index.html` - 大螢幕及手機登入介面 HTML5 結構。
- `style.css` - UI 毛玻璃風格、深色主題與微互動 CSS 樣式。
- `app.js` - 前端邏輯控制與 API 通訊。
- `raffle.js` - 幸運抽籤（純前端，加分沿用 `app.js` 的 `changeScore`）。
